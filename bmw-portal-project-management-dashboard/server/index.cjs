...
function main() {
  ...
  // createApp içinde json middleware yoksa garanti olsun (zararsız)
  ...
  app.post("/api/askgt/articles/upsert", (req, res) => {
    ...
    const nextItem = {
      ...
      tags: Array.isArray(incoming.tags) ? incoming.tags.map((t) => normalizeStr(t)).filter(Boolean) : [],
  ...
  app.listen(PORT, () => {
    ...
    console.log(`[AskGT] data file: ${ASKGT_FILE}`);
    console.log(`[Performance] mounted at /api/performance`);
  });
}
