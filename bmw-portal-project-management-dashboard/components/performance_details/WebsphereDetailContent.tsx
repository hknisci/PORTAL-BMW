...
const WebsphereDetailContent: React.FC<PerformanceDetailProps> = ({ payload }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <DetailKpiCard title="JVM Pools" value={fmt(k.jvmPools ?? "-")} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <PlaceholderDonutChart title="JVM Heap Pools" />
        <PlaceholderDonutChart title="HTTP Status Codes" />
      </div>

      <PlaceholderChart title="Transaction / Latency Trend" />
